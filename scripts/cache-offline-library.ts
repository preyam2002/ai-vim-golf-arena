...
const MAX_CHALLENGES = Number.isFinite(
  Number.parseInt(process.env.MAX_CHALLENGES || "")
)
...
async function main() {
  ...
  console.log(
    ...
    } challenges for models: ${TARGET_MODELS.join(", ")}${
  ...
  for (const challenge of challengeList) {
    ...
    for (const modelId of TARGET_MODELS) {
      ...
      try {
        ...
        console.log(
          `[ok] ${challenge.id} (${modelId}) -> ${result.keystrokeCount} keys, ${result.timeMs}ms`
        );
...
Do not include markdown, quotes, explanations, or extra lines.`;
  ...
  const tokenTimeline = buildTokenTimelineFromString(keystrokes, startTime);
  const cleaned = cleanKeystrokes(keystrokes);
  const simulator = new VimSimulator(challenge.startText);
  simulator.executeKeystrokes(cleaned);
...
function buildTokenTimelineFromString(
  keystrokes: string,
  startTime: number
): { token: string; timestampMs: number }[] {
  const tokens = cleanKeystrokes(keystrokes).split("");
  ...
  return tokens.map((token, index) => ({
    token,
    timestampMs: Date.now() - startTime + index * tokenDelayMs,
  }));
}
